...
function Edge(model, config){
	...
	self.drawSignals = function(ctx){
		...
		for(var i=0; i<self.signals.length; i++){
			...
			var toColor = Node.COLORS[self.to.hue] || Node.COLORS[self.from.hue];
			var blend;
	...
	self.update = function(speed){
		...
		dx = tx-fx;
		dy = ty-fy;
		w = Math.sqrt(dx*dx+dy*dy); // in other words, the distance formula
		a = Math.atan2(dy,dx); // arctangent of quotient of its arguments (y/x), returns angle in radians between positive x-axis of plane and point given by coords (x, y)
		h = Math.abs(self.arc*2);

		// From: http://www.mathopenref.com/arcradius.html
		r = (h/2) + ((w*w)/(8*h)); // the radius of our circle
	...
	self.draw = function(ctx){
		...
		// Stroke the arc with dynamic linewidth
		ctx.stroke();
		...
		// Stroke the arrow head
		ctx.stroke();

		// Draw the perpendicular lines with a constant linewidth
		ctx.lineWidth = 5; // Set the linewidth for the perpendicular lines to be constant
